...
<head>
    ...
    <title>Treatment Packages - Dashboard</title>
    ...
    <main class="dashboard-main">
        ...
        <div class="dashboard-content">
            ...
            <div class="page-header">
                <div>
                    <h2 style="margin: 0;">Your Treatment Packages</h2>
                    <p style="color: var(--text-muted); margin: var(--spacing-xs) 0 0;">Manage your treatment packages and pricing</p>
                </div>
                <a href="${pageContext.request.contextPath}/dashboard/packages/add" class="btn btn-primary">
            ...
            <c:choose>
                    ...
                    <div class="item-grid">
                        <c:forEach var="pkg" items="${packages}">
                            <div class="item-card">
                                <div class="item-image">
                                    <c:choose>
                                        <c:when test="${not empty pkg.imageUrl}">
                                            <img src="${pageContext.request.contextPath}${pkg.imageUrl}" alt="${pkg.packageName}">
                                        </c:when>
                                        <c:otherwise>
                                            <div style="width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; background: var(--neutral-sand);">
                                                <i class="fas fa-spa" style="font-size: 3rem; color: var(--text-muted);"></i>
                                            </div>
                                        </c:otherwise>
                                    </c:choose>
                                ...
                                <div class="item-body">
                                    <h4 class="item-title">${pkg.packageName}</h4>
                                    <div class="item-meta">
                                        <span><i class="fas fa-clock"></i> ${pkg.durationDays} Days</span>
                                        <span><i class="fas fa-tag"></i> ${pkg.packageType}</span>
                                    </div>
                                    <div style="margin-bottom: var(--spacing-md);">
                                        <span style="font-weight: 600; color: var(--primary-forest);">
                                            Starting ₹<fmt:formatNumber value="${pkg.budgetRoomPrice != null ? pkg.budgetRoomPrice : 0}" maxFractionDigits="0"/>
                                        </span>
                                    </div>
                                    <div class="item-actions">
                                        <a href="${pageContext.request.contextPath}/dashboard/packages/edit/${pkg.id}" class="btn btn-sm btn-secondary">
                                            <i class="fas fa-edit"></i> Edit
                                        </a>
                                        <form action="${pageContext.request.contextPath}/dashboard/packages/toggle/${pkg.id}" method="post" style="display: inline;">
                                            <button type="submit" class="btn btn-sm btn-outline">
                                                <i class="fas ${pkg.isActive ? 'fa-eye-slash' : 'fa-eye'}"></i>
                                            </button>
                                        </form>
                                        <form action="${pageContext.request.contextPath}/dashboard/packages/delete/${pkg.id}" method="post" style="display: inline;" onsubmit="return confirm('Are you sure you want to delete this package?');">
                ...
                <c:otherwise>
                    <div class="empty-state" style="background: white; padding: var(--spacing-3xl); border-radius: var(--radius-lg);">
                        ...
                        <p>Create your first treatment package to start receiving bookings</p>
                        <a href="${pageContext.request.contextPath}/dashboard/packages/add" class="btn btn-primary" style="margin-top: var(--spacing-lg);">
